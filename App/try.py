...
from kivymd.uix.button import MDButton,MDButtonText,MDIconButton,MDExtendedFabButtonIcon,MDExtendedFabButton,MDExtendedFabButtonText
...
def userForm():
    ...
    for i in fieldList:layout.add_widget(MDTextField())
    return scroll
    ...
    def __init__(self,**kwargs):
        ...
        root.add_widget(MDTopAppBar(MDTopAppBarLeadingButtonContainer(MDActionTopAppBarButton(icon='logout',on_release=lambda x:setattr(self.manager,'current','login'))),
            MDTopAppBarTitle(text='User Profiles',halign='center'),MDTopAppBarTrailingButtonContainer(
                MDActionTopAppBarButton(icon='account-circle-outline',on_release=lambda x:print('Menu')),
...
class LoginScreen(MDScreen):
    def __init__(self,**kwargs):
        ...
        loginCard = MDCard(orientation='vertical',md_bg_color=(0.12, 0.12, 0.12, 1),size_hint=(0.4,0.4),pos_hint={'center_x':0.5,'center_y':0.5},elevation=2)
        loginCard.add_widget(MDTextField(MDTextFieldLeadingIcon(icon='email'),MDTextFieldHelperText(text='Enter Your Email'),MDTextFieldHintText(text='Email')))
        loginCard.add_widget(MDTextField(MDTextFieldLeadingIcon(icon='lock'),MDTextFieldHelperText(text='Enter Your Password'),MDTextFieldHintText(text='Password'),MDTextFieldTrailingIcon(icon='')))
        loginCard.add_widget(MDIconButton(icon='home',on_release=lambda x:setattr(self.manager,'current','home')))
        loginCard.add_widget(MDExtendedFabButton(MDExtendedFabButtonIcon(icon='account'),MDExtendedFabButtonText(text='Admin'),on_release=lambda x:setattr(self.manager,'current','admin')))
        layout.add_widget(loginCard)
